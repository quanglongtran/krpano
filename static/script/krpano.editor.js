...
$(function () {
    ...
    $(".J-tool-btn").click(function () {
		...
		if ($(currentBtn).hasClass("btn-blue")) {
			$(currentBtn).removeClass("btn-blue")
			if ("sandTable" == $(currentBtn).attr("data-target")) {
				krpano.call("closemap()");
			}
			
		} else {
			$(currentBtn).addClass("btn-blue");
			$("[name=" + $(currentBtn).attr("data-target") + "]").show();
...
function onready() {
	...
	//SANDTABLE列表模块
	var SandSpotHTML;
	var dataSandSpotList = {list: []};
	krpano.get("layer").getArray().forEach(function (oldLayer) {
        if (oldLayer.name.substr(0,8) == 'sandspot') {
            dataSandSpotList.list.push(oldLayer);
        }
    });
	SandSpotHTML = template('tplSandSpotList', dataSandSpotList);
    document.getElementById('sandSpotList').innerHTML = SandSpotHTML;
...
//提交SAND修改模块
function updateSandSpotData() {
    var sandSpotHTML;
    var dataSandSpotList = {list: []};
    //修改全局变量
    var sandSpotData = [];
    krpano.get("layer").getArray().forEach(function (everyLayer) {
        if (everyLayer.name.substr(0,8) == 'sandspot') {
            dataSandSpotList.list.push(everyLayer);
            var sandSpot = {};
            sandSpot.url = everyLayer.url;
            sandSpot.align = everyLayer.align;
            sandSpot.parent = everyLayer.parent;
            sandSpot.x = everyLayer.x;
            sandSpot.y = everyLayer.y;
            sandSpot.onclick = everyLayer.onclick;
            sandSpotData.push(sandSpot);
        }
    });
	sandSpotHTML = template('tplSandSpotList', dataSandSpotList);
    document.getElementById('sandSpotList').innerHTML = sandSpotHTML;
...
function save() {
	...
	var postObject = []
	postObject[0] = sceneList
	
	var layerList = []
	krpano.get("layer").getArray().forEach(function (oldLayer) {
        if (oldLayer.name.substr(0,8) == 'sandspot') {
            layerList.push({name: oldLayer.name,
							keep:"true",
							url:"skin/test.png",
							align:"lefttop",
							align:"lefttop",
							...
							y:"108",
							ondown:"draglayer()",
							onclick:"loadscene(test2, null, MERGE, BLEND(1));",
							parent:"map"
			});
        }
    });
...
//添加layer模块(for SandSpot)
function showAddSandSpot() {
	console.log("showAddSandSpot")
    $(".hot-style").removeClass("hot-style-on");
    $(".hot-style").first().addClass("hot-style-on");
    toAddSandSpot.style = $(".hot-style").first().attr("name");
    toAddSandSpot.dive = true;
    $(".my-open-out").css("background", "#00a3d8");
    $(".my-open-in").removeClass("my-open-in-close").addClass("my-open-in-open");
    $("#SandSpotButton").hide();
    $(".add-sand-pot").show();
    $("#selectStyleSand").show();
    $("#goToSceneSand").hide();
    $("#writeTitle").hide();
    $("#selectStyleTitleSand").addClass("progress-title-on");
    $("#goToSceneTitleSand").removeClass("progress-title-on");
    $("#writeTitleTitleSand").removeClass("progress-title-on");
}
...
function hideAddSandSpot() {
    window.clearInterval(moveIntervalId);
    toAddSandSpot = {};
    $(".add-sand-pot").hide();
    $("span[data-target=#sandSpot]").click();
}
...
function nextToSelectTargetSceneSand() {
    if (!toAddSandSpot.style) {
        alert("请选择SANDMAP热点样式");
        return
    }
    ...
    targetSceneHTML = template('tplTargetSceneSand', dataTargetScene);
    document.getElementById('targetSceneSand').innerHTML = targetSceneHTML;

    $(".select-scene-div").removeClass("select-scene-div-on");
    $(".select-scene-div").first().addClass("select-scene-div-on");
    toAddSandSpot.linkedscene = $(".select-scene-div").first().attr("name");
    $(".select-scene-div").unbind("click").click(function () {
        toAddSandSpot.linkedscene = $(this).attr("name");
        $(".select-scene-div").removeClass("select-scene-div-on");
        $(this).addClass("select-scene-div-on");
    });

    $("#selectStyleSand").hide();
    $("#goToSceneSand").show();
    $("#writeTitleSand").hide();
    $("#goToSceneTitleSand").addClass("progress-title-on");
    $("#selectStyleTitleSand").removeClass("progress-title-on");
    $("#writeTitleTitleSand").removeClass("progress-title-on");
}
...
function nextToWriteTitleSand() {
    if (!toAddSandSpot.linkedscene) {
        alert("请选择SAND目标场景");
        return;
    }

    $("#addSandTitle").val(toAddSandSpot.linkedscene);

    $("#selectStyleSand").hide();
    $("#goToSceneSand").hide();
    $("#writeTitleSand").show();
    $("#writeTitleTitleSand").addClass("progress-title-on");
    $("#selectStyleTitleSand").removeClass("progress-title-on");
    $("#goToSceneTitleSand").removeClass("progress-title-on");
}
...
function addSandSpot() {
    //添加热点
    var newSandSpotName = "sandspot_" + new Date().getTime();
    krpano.call("addlayer(" + newSandSpotName + ");");
    krpano.set("layer[" + newSandSpotName + "].url", "skin/spot.png");
    krpano.set("layer[" + newSandSpotName + "].align", "bottom");
    krpano.set("layer[" + newSandSpotName + "].parent", "map");
    krpano.set("layer[" + newSandSpotName + "].x", 300);
    krpano.set("layer[" + newSandSpotName + "].y", 120);
	krpano.set("layer[" + newSandSpotName + "].onclick", "loadscene(test2, null, MERGE, BLEND(1));");
	krpano.set("layer[" + newSandSpotName + "].ondown", "draglayer()");
    sandSpotInitEvent(newSandSpotName);
    updateSandSpotData();
    $("#isEdited").text('保存*');
    hideAddSandSpot();
}
...
function sandSpotInitEvent(spotName) {
